<template>
  <div class="fixed z-10 w-full backdrop-blur-md" :class="{
    'bg-white': !isHomePage,
  }">
    <div class="py-3">
      <AppContainer>
        <div class="flex items-center justify-between">
          ...
          <div class="flex items-center space-x-4">
            <NavbarUserMenu />
            <Button variant="ghost" size="icon" class="md:hidden" :class="{
        ...
        <div class="fixed inset-y-0 right-0 z-50 w-full max-w-sm border-l shadow-lg bg-background">
          <div class="flex flex-col h-full">
            ...
            <nav class="flex flex-col p-4 space-y-4">
              <NuxtLink v-for="item in mainMenu" :key="item.path" :to="item.path" @click="toggleMobileMenu"
