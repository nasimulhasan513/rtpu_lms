<template>
  <AppContainer>
    <div class="container mx-auto py-8 px-4">
        ...
        <div class="md:col-span-2">
          ...
          <div class="flex items-center mb-4">
            <span class="text-2xl font-semibold text-primary mr-4"
              >৳{{ book.sale_price }}</span
            >
            <span
              v-if="book.regular_price > book.sale_price"
              class="text-lg text-muted-foreground line-through"
              >৳{{ book.regular_price }}</span
            >
          </div>

          <p class="text-lg mb-4">{{ book.short_description }}</p>
          ...
          <div class="mb-4">
            ...
            <ul class="list-disc list-inside">
              <li
                v-for="(desc, index) in book.description"
                :key="index"
                class="mb-2"
              >
                {{ desc }}
      ...
      <!-- Preview Images -->
      <div class="mt-12">
        <h2 class="text-2xl font-semibold mb-4">Preview Pages</h2>
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
          <img
            v-for="(preview, index) in book.preview"
            :key="index"
            :src="preview"
            :alt="`Preview page ${index + 1}`"
            class="w-full rounded-lg shadow cursor-pointer hover:opacity-80 transition-opacity"
