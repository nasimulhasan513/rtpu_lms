<template>
    <AppContainer class="py-12 min-h-[76vh]">
        <Card class="max-w-3xl p-6 mx-auto shadow-md rounded-xl">
            <div class="flex flex-col items-center gap-4 mb-6 sm:flex-row sm:items-start">
                <ClientOnly>
                    <AuthProfilePic />
                </ClientOnly>
                <div class="flex flex-col items-center flex-1 gap-1 text-center sm:items-start sm:text-left">
                    <div class="text-2xl font-bold">{{ user.name }}</div>
                    <div class="text-muted-foreground">{{ user.phone }}</div>
                    <div class="text-sm text-muted-foreground">
                        <Icon v-if="user.google_id" name="logos:google-icon" class="inline-block mr-1" />
                        {{ user.email }}
                    </div>
                </div>
            </div>
