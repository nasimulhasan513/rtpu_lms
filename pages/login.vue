    ...
    <div class="grid max-w-sm gap-6 mx-auto">
      <div class="grid gap-2 text-center">
        ...
        <p class="text-sm text-balance text-muted-foreground">
          Welcome to Academia! Please login to continue.
        </p>
      </div>
      <div class="space-y-6">
        ...
        <div class="space-y-6">
          <FormField v-slot="{ componentField }" name="cred">
            <FormItem>
              <FormLabel>
                {{ credType ? '' : 'E-mail or Phone Number' }}
                <span v-if="credType">
                  {{ credType == 'email' ? 'E-mail Address' : 'Phone Number' }}
                </span>
              </FormLabel>
              <FormControl>
                <Input type="text" placeholder="E-mail or Phone number" v-bind="componentField" v-model="cred"
          ...
          <FormField v-if="step === 'signup'" v-slot="{ componentField }" name="cred2">
            <FormItem>
              <FormLabel>{{ credType == 'email' ? 'Phone Number' : 'E-mail Address' }}</FormLabel>
              <FormControl>
                <Input type="text" :placeholder="credType == 'email' ? 'Phone Number' : 'E-mail Address'"
          ...
          <FormField v-if="step === 'login' || step === 'signup'" v-slot="{ componentField }" name="password">
            <FormItem>
              <FormLabel> {{ step == 'login' ? '' : 'New Account ' }} Password</FormLabel>
              <FormControl>
                <div class="relative">
                  <Input :type="showPass ? 'text' : 'password'" placeholder="********" v-bind="componentField"
                    v-model="password" />
          ...
          <FormField v-if="step === 'otp'" v-slot="{ componentField }" name="otp">
            <FormItem>
              ...
              <FormControl>
                <Input type="tel" placeholder="OTP" v-bind="componentField" v-model="otp" />
              </FormControl>
              <FormMessage />
            </FormItem>
          </FormField>
        ...
        <div class="flex flex-col gap-2">
          <div class="flex flex-row items-center w-full gap-4">
            ...
            <AppButton :label="buttonLabels[step]" :loadingLabel="buttonLoadingLabels[step]" :loading="isLoading"
